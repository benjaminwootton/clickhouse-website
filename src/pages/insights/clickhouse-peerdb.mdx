...
There are two main mechanisms for integrating data.  

Firstly, we have CDC (Change Data Capture), where PeerDB will listen to a log of low level change events from Postgres.  

Secondly, there is a SQL Query Based integration which gives us the option of specifying our source as a SQL query, this giving us the option of selecting a subset of rows and potentially doing joins and transformations at source.  This requires some ID or timestamp to monitor which rows have been synced in each batch and would typically add more load onto the source database than the CDC approach so is arguably less preferred.   
...
The QuickStart guide creates a Postgres database called _source_ which contains a table called _test_.  We can use that to initially experiment with PeerDB.
...
If you need to create any test data in Postgres, you can use the following technique:
