...
In this article we are going to demonstrate how we can use [PeerDB](https://peerdb.io) to integrate data from Postgres to ClickHouse.  We will discuss both a self hosted open source stack and a fully managed solution using ClickHouse Cloud.   

### Why Postgres and ClickHouse?

Many data teams use ClickHouse to complement Postgres.  Typically, they start with Postgres as their transactional application database, but over time find that the analytical elements of their user experience slow down.  To combat this, they decide to add a separate OLAP database more suited to analytics, and often opt for ClickHouse.  
...
Secondly, in July 2024 [ClickHouse acquired PeerDB](https://clickhouse.com/blog/clickhouse-acquires-peerdb-to-boost-real-time-analytics-with-postgres-cdc-integration).  With PeerDB under the ClickHouse umbrella, the integration between the two is deepening such as the [private preview of PeerDB that has been integrated into ClickHouse cloud](https://clickhouse.com/blog/postgres-cdc-connector-clickpipes-private-preview).  This means that PeerDB is the strategic and obvious choice when working with these two technologies.
...
It will integrate all inserts, updates and deletes that occur against selected source tables and apply them to the target continuously and in near real-time.   This means that we can maintain a copy of selected and relevant data in ClickHouse which should accurately mirror the Postgres source table at any given time.

[Transformations can be applied within PeerDB](https://blog.peerdb.io/row-level-transformations-in-postgres-cdc-using-lua) as data moves between the two systems, defined using a simple scripting language.  Performing transformations within PeerDB can keep load off the databases and support use cases such as removing sensitive data or unnesting JSON during the ETL process.
...
There are two options for integrating Postgres and ClickHouse which are referred to as CDC and SQL Based.

CDC (Change Data Capture) is where PeerDB will listen to a log of low level change events emitted from Postgres and replicate them to ClickHouse.  

The SQL Query Based extracts data from the source through a SQL query.  This allows you to do things like select a subset of rows from the table, carry out joins across multiple source tables and apply other transformations at source.  

The SQL Query route typically adds more load onto the source database than the CDC approach so is arguably second choice even though it is more flexible.   With a fast analytical database like ClickHouse and the option of applying transformations within PeerDB, it may be better to stick to CDC and do any transformation work within ClickHouse away from your production transactional database.

### Open Source vs PeerDB Cloud vs ClickHouse Cloud

As of December 2024, you have three options for making use of PeerDB.

The first is to use the open source PeerDB tools and run it in a self hosted configuration on your own server.  PeerDB is distributed as a set of Docker containers and ran using a Docker compose file making it relatively easy to deploy and run. 

Secondly, you can use PeerDB cloud, which is a hosted version of PeerDB hosted by PeerDB themselves.  Though this has traditionally supported other databases such as Snowflake and BigQuery, as of July 2024 it only supports ClickHouse Cloud as a destination.  

Finally, you can use afully managed version which is integrated and embedded into the ClickHouse Cloud UI.  In this instance, it's labelled as ClickPipes, though PeerDB is likely used behind the scenes.  
...
We will start with a runthrough as to how to self manage PeerDB open source.
...
At this point, if everything is configured correctly, PeerDB should be ready to move data from Postgres to ClickHouse for your selected tables.  You can monitor the status of the replication process in the UI or logs.

### Source Table In Postgres

The QuickStart guide creates a Postgres database called __source__ which contains a table called __test__.  We can use that table to experiment with PeerDB by mirroring it into ClickHouse.  The table has the following schema:
...
Within a minute, the data should be replicated into ClickHouse, ingested as a single table matching the Postgres schema.  Columns for the sync time, the deleted flag and the peerdb version are added automatically by PeerDB:
...
Aside from these gotchas, the Open Source PeerDB version was relatively easy to deploy and run and we found it scalable to millions of rows with low latency.  

### Cloud Hosted Solution 

As mentioned, we have two options for a managed solution.  The first is PeerDB Cloud and the second is to use ClickHouse Cloud which now integrates PeerDB to provide it's Postgres CDC solution.  

As of December 2024, you still need to [request access to the private preview via support](https://clickhouse.com/cloud/clickpipes/postgres-cdc-connector).
