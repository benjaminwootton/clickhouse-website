...
Firstly, where many of the ETL tools have a wide range of connectors, PeerDB chose to focus exclusively on Postgres as a data source.  This focus means that they have been able to build one of the fastest and most scalable CDC solutions for Postgres, whilst also supporting it's advanced features such as it's complex data types and partitioned tables.

Secondly, in July 2024 [ClickHouse acquired PeerDB](https://clickhouse.com/blog/clickhouse-acquires-peerdb-to-boost-real-time-analytics-with-postgres-cdc-integration) to make integrating Postres and ClickHouse easier.  With PeerDB under the ClickHouse umbrella, the integration between the two is deepening.  For instance, a [private preview of PeerDB integrated inside ClickHouse cloud is now available](https://clickhouse.com/blog/postgres-cdc-connector-clickpipes-private-preview).  This means that PeerDB is the strategic choice when working with these two technologies.

### How PeerDB Works 

PeerDB is responsible for taking data from a Postgres source (including both open source and managed versions such as AWS RDS and Google Cloud) and copying the data into a target data warehouse, in our case ClickHouse.  

It will integrate all inserts, updates and deletes that occur against selected source tables and can run continuously and in near real-time.   This means that we can maintain a copy of selected and relevant data in ClickHouse which should accurately mirror the source table at any given time.

The two main mechanisms for integrated data.  Firstly, we have CDC (Change Data Capture), where PeerDB will listen to a log of low level change events from Postgres.  Secondly, there is a SQL Query Based integration where we can select a subset of rows and do joins and transformations prior to the integration.  The latter relies on some ID or timestamp to monitor which rows have been synced in each batch and would typically add more load onto the source database than the CDC approach so is arguably less preferred.   

Additional [transformations can be applied directly within PeerDB](https://blog.peerdb.io/row-level-transformations-in-postgres-cdc-using-lua) as data moves between the two systems.  This can keep load off the databases and support use cases such as removing sensitive data or unnesting JSON during the ETL process.
...
We won't repeat all of the [PeerDB documentation here](https://docs.peerdb.io/quickstart/quickstart) as the QuickStart guide is clear.  However, there are four steps:
...
- Access the PeerDB UI, configure the source and target peer details, then configure a mirror to integrate the two peers.
...
If you need to create any test data in Postgres for testing, you can use the following technique:
...
The data should be represented in ClickHouse as a single table matching the PeerDB configuration.  Columns for the sync time, the deleted flag and the peerdb version are added automatically by PeerDB:
...
If you update a row, a new row is created in your ClickHouse target table.  If you choose a ReplacingMergeTree, the new row will effectively replace the old row giving you update semantics in your target table.  
...
Deletes in Postgres are sent into the target with a peerdb_is_deleted column set to 1, replicating a logical delete.  You would therefore need to handle the fact that the record is logically deleted either in your downstream ClickHouse views, reports or applications.  
...
If you add a column on your source table, this is automatically reflected in ClickHouse at the point that the next record is integrated.  In the example below, we have added a new column c4 into Postgres, and found this to be created in ClickHouse with a default value:
...
The first is related to the MiniIO Configuration.  PeerDB takes data from Postgres and puts it in a staging area before it is loaded into ClickHouse.  By default this staging area is hosted in a [MinIO](https://min.io/) container.  It is important to set the hostname which both PeerDB and ClickHouse can access in the docker compose file.  By default it is set to host.docker.internal in the docker-compose file which will not be accessible to ClickHouse.

```
PEERDB_CLICKHOUSE_AWS_CREDENTIALS_AWS_ENDPOINT_URL_S3: http://172.31.26.57:9001
```

Secondly, we experienced a number of issues relating to Docker Snap on Ubuntu.  This included containers in Docker compose not being able to connect to each other, and not being able to write to our external EBS volume with more disk.  

Aside from this, the Open Source PeerDB version was relatively easy to deploy and run.

### Cloud Hosted Solution 

PeerDB has now been integrated into ClickHouse Cloud, though as of December 2024, you still need to request access to the private preview via support.

The process for creating the link is similar to the open source UI.  You create a ClickPipe associated with your ClickHouse Cloud instance and select the Postgres CDC option.  You can then specify the details for your source database, choose an area within your ClickHouse Cloud instance to write to.
