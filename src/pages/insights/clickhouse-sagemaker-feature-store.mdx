...
In a [recent blog post](/blog/forecasting-using-clickhouse-sagemaker) we walked through the process of combining [AWS SageMaker](https://aws.amazon.com/sagemaker/) with ClickHouse to develop and deploy a machine learning model.  
...
In this article we wanted to dig deeper into one element of that - the feature store.  This article builds on a recent post by the ClickHouse team [Powering Feature Stores With ClickHouse](/blog/powering-featurestores-with-clickhouse), but shows a slightly different approach and technology stack based on SageMaker, the AWS machine learning platform.  
...
- We could have made use of a virtual feature store product such as Featureform as outlined in the [ClickHouse blog post](/blog/powering-featurestores-with-clickhouse) to manage this process.
...
Together with our first blog post, we hope we are making the point that ClickHouse [is a great fit for supporting the MLOps workflow](/blog/forecasting-using-clickhouse-sagemaker)! 
